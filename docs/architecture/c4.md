...
| -------------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------- |
| `api/routes/*`             | Registra endpoints, parseia `If-Match` e UUID, coordena request e response.     | `TicketUseCases`                                    |
| `api/http/*`               | Serializa Ticket, calcula ETag e converte falhas em `application/problem+json`. | Fastify                                             |
| `api/dependencies.ts`      | Único ponto HTTP que escolhe e conecta adaptadores concretos.                   | `TicketApplicationService`, PostgreSQL              |
| `TicketApplicationService` | Fachada dos casos de uso; depende dos contratos de comando e consulta.          | `TicketCommandRepository`, `TicketQueryRepository`  |
...
| `PostgresTicketRepository` | Transações, idempotência, histórico, paginação e intenção Outbox.               | Drizzle/PostgreSQL                                  |

As rotas não importam infraestrutura diretamente; esse limite é verificado por
